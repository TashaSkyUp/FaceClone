...
import insightface
import roop.globals
from roop.analyser import get_face_many, get_face_idx
...
def process_faces(source_face, frame, progress, face_idx: tkinter.IntVar = None):
    face_idx = face_idx.get()
    if not face_idx:
        ...
        face = get_face_idx(frame, face_idx)
        if face:
            frame = swap_face_in_frame(source_face, face, frame)
            progress.set_postfix(status='.', refresh=True)
        else:
...
def process_video(source_img, frame_paths):
    source_face = get_face_idx(cv2.imread(source_img), 0, allow_teleport=True)
    ...
    with tqdm(total=len(frame_paths), desc="Processing", unit="frame", dynamic_ncols=True,
        ...
        for frame_path in frame_paths:
            ...
            try:
                result = process_faces(source_face, frame, progress, roop.globals.face_idx)
...
def process_img(source_img, target_path, output_file):
    ...
    face = get_face_idx(frame, roop.globals.face_idx.get())
    source_face = get_face_idx(cv2.imread(source_img), 0, allow_teleport=True)
